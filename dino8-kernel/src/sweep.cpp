...
// Rotation-minimizing transport of a reference direction along a sequence
// of EXPLICIT (origin, unit direction) pairs - the same double-reflection
// method RmfFrames() uses along a rail curve's own tangent (Wang et al.
// 2008), generalized to any tangent-like direction series. Sweep2() below
// transports the CHORD direction between two rails, not a single curve's
// tangent, so it cannot sample TangentAt() the way RmfFrames() does and
// needs its own copy of the same transport loop. `wrap` spreads the same
// holonomy twist correction over the stations so the last frame meets the
// first.
struct ChordFrame {
  ON_3dPoint origin;
  ON_3dVector x, y, z;  // x given; y transported; z = x cross y
};

std::vector<ChordFrame> ChordTransportFrames(const std::vector<ON_3dPoint>& origins,
                                             const std::vector<ON_3dVector>& x_dirs, bool wrap, const char* caller) {
  const int m = static_cast<int>(origins.size());
  std::vector<ChordFrame> frames(static_cast<size_t>(m));
  for (int k = 0; k < m; ++k) {
    frames[static_cast<size_t>(k)].origin = origins[static_cast<size_t>(k)];
    frames[static_cast<size_t>(k)].x = x_dirs[static_cast<size_t>(k)];
  }
  // Initial reference direction: the world axis least aligned with x0.
  {
    const ON_3dVector x0 = frames[0].x;
    ON_3dVector a = std::fabs(x0.x) <= std::fabs(x0.y) && std::fabs(x0.x) <= std::fabs(x0.z) ? ON_3dVector(1, 0, 0)
                    : std::fabs(x0.y) <= std::fabs(x0.z)                                    ? ON_3dVector(0, 1, 0)
                                                                                            : ON_3dVector(0, 0, 1);
    ON_3dVector y0 = a - x0 * ON_DotProduct(a, x0);
    y0.Unitize();
    frames[0].y = y0;
  }
  for (int k = 0; k + 1 < m; ++k) {
    const ChordFrame& f = frames[static_cast<size_t>(k)];
    ChordFrame& g = frames[static_cast<size_t>(k + 1)];
    const ON_3dVector v1 = g.origin - f.origin;
    const double c1 = ON_DotProduct(v1, v1);
    if (c1 <= 0.0) Fail(caller, "two consecutive sweep stations coincide");
    const ON_3dVector yL = f.y - v1 * (2.0 / c1 * ON_DotProduct(v1, f.y));
    const ON_3dVector xL = f.x - v1 * (2.0 / c1 * ON_DotProduct(v1, f.x));
    const ON_3dVector v2 = g.x - xL;
    const double c2 = ON_DotProduct(v2, v2);
    ON_3dVector y = c2 > 0.0 ? yL - v2 * (2.0 / c2 * ON_DotProduct(v2, yL)) : yL;
    y = y - g.x * ON_DotProduct(y, g.x);  // re-orthogonalize against rounding
    y.Unitize();
    g.y = y;
  }
  if (wrap && m >= 2) {
    const ChordFrame& f = frames.back();
    const ChordFrame& g0 = frames.front();
    const ON_3dVector v1 = g0.origin - f.origin;
    const double c1 = ON_DotProduct(v1, v1);
    if (c1 <= 0.0) Fail(caller, "the closed sweep's last station coincides with its first");
    const ON_3dVector yL = f.y - v1 * (2.0 / c1 * ON_DotProduct(v1, f.y));
    const ON_3dVector xL = f.x - v1 * (2.0 / c1 * ON_DotProduct(v1, f.x));
    const ON_3dVector v2 = g0.x - xL;
    const double c2 = ON_DotProduct(v2, v2);
    ON_3dVector y = c2 > 0.0 ? yL - v2 * (2.0 / c2 * ON_DotProduct(v2, yL)) : yL;
    y = y - g0.x * ON_DotProduct(y, g0.x);
    y.Unitize();
    const ON_3dVector z0 = ON_CrossProduct(g0.x, g0.y);
    const double twist = std::atan2(ON_DotProduct(y, z0), ON_DotProduct(y, g0.y));
    for (int k = 1; k < m; ++k) {
      ChordFrame& fk = frames[static_cast<size_t>(k)];
      const double a = -twist * static_cast<double>(k) / m;
      const ON_3dVector zk = ON_CrossProduct(fk.x, fk.y);
      fk.y = fk.y * std::cos(a) + zk * std::sin(a);
    }
  }
  for (ChordFrame& f : frames) f.z = ON_CrossProduct(f.x, f.y);
  return frames;
}
...
Brep Brep::Sweep2(const NurbsCurve& section_in, const NurbsCurve& rail1_in, const NurbsCurve& rail2_in, int stations) {
  ...
  if (section.IsClosed()) {
    Fail(caller,
         "section must be an open curve - its two distinct endpoints are the ones matched to rail1 and rail2");
  }
  const bool wrap1 = rail1.IsClosed(), wrap2 = rail2.IsClosed();
  if (wrap1 != wrap2) Fail(caller, "rail1 and rail2 must both be open or both be closed");
  const bool wrap = wrap1;
  const int m = std::max(stations, 3);

  // Equal-arc-length stations on EACH rail independently (the wrapper's
  // DivideByCount, same convention Sweep1()/PipeVariable() use): station k
  // is the same fraction of rail1's own arc length and of rail2's own arc
  // length - the "same parameter" rail correspondence.
  std::vector<double> params1 = rail1_in.DivideByCount(wrap ? m : m - 1);
  std::vector<double> params2 = rail2_in.DivideByCount(wrap ? m : m - 1);
  if (wrap) {
    params1.pop_back();
    params2.pop_back();
  }
  if (static_cast<int>(params1.size()) != m || static_cast<int>(params2.size()) != m) {
    Internal(caller, "station count mismatch");
  }

  const ON_3dPoint O0 = section.PointAtStart();
  const ON_3dVector chord0 = section.PointAtEnd() - O0;
  const double L0 = chord0.Length();
  if (!(L0 > 0.0)) {
    Fail(caller,
         "the section's start and end points coincide - Sweep2 needs an open profile whose two ends are matched "
         "to rail1 and rail2");
  }
  const ON_3dVector X0 = chord0 / L0;

  std::vector<ON_3dPoint> origins(static_cast<size_t>(m));
  std::vector<ON_3dVector> x_dirs(static_cast<size_t>(m));
  std::vector<double> scale_k(static_cast<size_t>(m));
  for (int k = 0; k < m; ++k) {
    const ON_3dPoint p1 = rail1.PointAt(params1[static_cast<size_t>(k)]);
    const ON_3dPoint p2 = rail2.PointAt(params2[static_cast<size_t>(k)]);
    const ON_3dVector v = p2 - p1;
    const double len = v.Length();
    if (!(len > 0.0)) Fail(caller, "rail1 and rail2 meet (zero separation) at station " + std::to_string(k));
    origins[static_cast<size_t>(k)] = p1;
    x_dirs[static_cast<size_t>(k)] = v / len;
    scale_k[static_cast<size_t>(k)] = len / L0;
  }
  const std::vector<ChordFrame> frames = ChordTransportFrames(origins, x_dirs, wrap, caller);

  // Station k's copy: scale `section` about its own start point O0 by
  // scale_k[k] (so O0 -> O0, PointAtEnd() -> O0 + scale_k[k] * L0 * X0),
  // then rotate/translate the (O0, X0, frames[0].y) frame onto
  // (frames[k].origin, frames[k].x, frames[k].y) - carrying O0 exactly to
  // rail1's station point and PointAtEnd() exactly to
  // O0 + scale_k[k] * L0 * frames[k].x = frames[k].origin +
  // |rail2(k) - rail1(k)| * frames[k].x, i.e. exactly to rail2's station
  // point, at every station.
  const ON_Plane plane0(O0, X0, frames[0].y);
  std::vector<ON_NurbsCurve> copies;
  copies.reserve(static_cast<size_t>(m));
  for (int k = 0; k < m; ++k) {
    ON_NurbsCurve ck = section;
    const ON_Xform scale = ON_Xform::ScaleTransformation(O0, scale_k[static_cast<size_t>(k)]);
    const ON_Plane planek(frames[static_cast<size_t>(k)].origin, frames[static_cast<size_t>(k)].x,
                          frames[static_cast<size_t>(k)].y);
    ON_Xform rot;
    rot.Rotation(plane0, planek);
    ck.Transform(rot * scale);
    copies.push_back(std::move(ck));
  }
  MakeCompatible(copies, caller);  // no-op for these copies (same knots/degree/rationality); one code path

  double period = 1.0;
  const std::vector<double> params_v = SkinParameters(copies, wrap, &period, caller);
  std::unique_ptr<ON_NurbsSurface> wall = SkinSections(copies, std::min(3, m - 1), wrap, params_v, period, caller);
  return AssembleSweptBody(wall.release(), false, false, false, false, caller);
}
