...
| Blending & chamfering | 1.5 | 24 | 5 | 16 | 3 | 54.2% |
| kernel: Sweeping, lofting, extruding, revolving | 1 | 29 | 6 | 19 | 4 | 53.4% |
...
- [missing] Face-face imprint (Parasolid PK_BODY_imprint / ACIS imprint: split faces along mutual intersection without removing material) — re-grepped `imprint`: none. The general engine's internal face-splitting (SplitFaceLoop) is not exposed as an operation.
...
- [missing] Conic / rho (chordal, elliptical) blend cross-sections — re-grepped `conic|rho|RailType`: no hits outside an unrelated font-outline helper. Cross-sections are circular, ruled or Hermite only.
...
- [missing] Sweep controls: twist along path, scale along path, road-like / fixed-up alignment (AutoCAD SWEEP Twist/Scale/Alignment, Rhino Roadlike/Frame rotate) — neither `Brep::Sweep1` nor the app's `Sweep1Command` has twist, scale or road-like frame options; Sweep2's rail-driven scaling is not a user control.
...
- [kernel/sweeplofts] Sweep controls: twist/scale/roadlike alignment (missing)
