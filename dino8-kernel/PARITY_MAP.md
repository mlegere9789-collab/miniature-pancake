...
- [partial] Split body with an arbitrary surface / solid cutter (BooleanSplit with a cutting object, PK_BODY_section by a sheet) — the app now has a real cutting-object split: `SplitByObjectCommand` (cmd_boolean.cpp, registered as `SplitByObject`) selects solids to split, then one or more cutting solids or open surfaces/meshes; a closed cutter is used as-is, an open one is solidified (extruded from its own position along its area-weighted average normal far enough to clear every target), multiple cutters are unioned into one tool, and each target is cut with `BooleanCombine(target, tool, Intersection)`/`Difference`. Still partial: the open-cutter path is a single-normal-direction approximation (not a true trim), and no PK_BODY_section-style face-by-face imprinting/healing is attempted - this is a mesh boolean, like every other `Boolean*` command here.
