...
class NurbsSurface {
 public:
  ...
  // answer, about 8.7% of the radius). A second real, reproduced bug of
  // the same "window locks onto the wrong place" family, at a DEGENERATE
  // edge (a sphere's pole, a cone's apex, a revolved surface's on-axis
  // end): every u sample on the pole row evaluates to the same point, so
  // the coarse scan's best u there was just the first tied sample, and
  // narrowing the u window around it locked every later level into the
  // wrong azimuth - a query 0.3r off the surface, 0.36 degrees from the
  // pole, came back ~32 degrees of azimuth away. Fixed by not narrowing a
  // direction whose row/column of samples through the best point is flat
  // (identical distances to 1e-12 relative) at that level, so the other
  // direction moves off the degenerate edge first.
