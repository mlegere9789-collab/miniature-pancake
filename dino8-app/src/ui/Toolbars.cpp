...
void DrawToolbars(Application& app) {
  ...
  if (ImGui::Begin("##toolbar", nullptr, flags)) {
    ...
    for (const ToolButton& b : kButtons) {
      if (!b.command) {
        ImGui::SameLine();
        ImGui::TextDisabled("|");
        ImGui::SameLine();
      ...
      const RegisteredCommand* rc = app.Engine().Find(b.command);
      ...
      if (running && rc && rc->name == app.Engine().ActiveName()) {
        ...
        if (ImGui::Button(b.label)) app.Engine().Execute(b.command);
        ImGui::PopStyleColor();
      } else if (ImGui::Button(b.label)) {
        app.Engine().Execute(b.command);
      }
      if (!implemented) ImGui::PopStyleColor();
      if (ImGui::IsItemHovered(ImGuiHoveredFlags_DelayShort)) ImGui::SetTooltip("%s  (%s)", b.tip, b.command);
      ImGui::SameLine();
    }
    ImGui::NewLine();
  }
