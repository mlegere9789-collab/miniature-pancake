...
class GlRenderer {
 public:
  ...
  // Lines: x,y,z pairs.
  void DrawLines(const std::vector<float>& data, Color color);
 ...
 private:
  ...
  GLuint vao_ = 0, vbo_ = 0, bg_vao_ = 0;
  Mat4 view_ = Mat4::Identity(), proj_ = Mat4::Identity();
  kernel::Vector3d light_{0.3, 0.5, 1.0};
  GLint mesh_u_mvp_ = -1, mesh_u_view_ = -1, mesh_u_color_ = -1, mesh_u_light_ = -1, mesh_u_lit_ = -1;
  GLint line_u_mvp_ = -1, line_u_color_ = -1, line_u_size_ = -1;
