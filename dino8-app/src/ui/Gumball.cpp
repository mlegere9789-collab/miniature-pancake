...
bool Gumball::Update(Application& app, Viewport& vp, bool viewport_hovered) {
  ...
  if (!dragging_) {
    ...
    if (hover_ != Handle::None && ImGui::IsMouseClicked(0)) {
      ...
      if (drag_handle_ == Handle::Free) {
        ...
        start_param_ = ClosestParamOnLine(ray, center_, AxisDir(static_cast<int>(drag_handle_) - 1));
      }
    }
  }
  ...
  if (dragging_) {
    ...
    if (drag_handle_ == Handle::Free) {
      ...
      const Vector3d dir = AxisDir(static_cast<int>(drag_handle_) - 1);
      delta = dir * (ClosestParamOnLine(ray, center_, dir) - start_param_);
    }
    if (app.Snaps().grid_snap) {
      const double g = doc.Settings().grid_spacing;
      delta = Vector3d(std::round(delta.x / g) * g, std::round(delta.y / g) * g, std::round(delta.z / g) * g);
    }
    const ON_Xform xf = ON_Xform::TranslationTransformation(delta);
    ...
    if (!ImGui::IsMouseDown(0)) {
      dragging_ = false;
      originals_.clear();
      app.Engine().Print("Gumball: moved " + FormatPoint(Point3d(delta.x, delta.y, delta.z)));
      doc.Touch();
    }
  }
