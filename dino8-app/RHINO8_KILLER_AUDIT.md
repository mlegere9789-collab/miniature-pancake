...
- [x] Expand Dino Flow: data-tree semantics (lists/graft/flatten) already existed in `src/flow/FlowData.{h,cpp}`/`FlowNodesCore.cpp` (Tree/Branch, Graft/Flatten/Simplify Tree, List Item/Length) and now has test coverage; added a Galapagos-style evolutionary solver node family (`Gene Pool` + `Evolutionary Solver`, `src/flow/FlowNodesSolver.cpp` + `RunSolverNode()` in `FlowGraph.cpp`) and three more real example plug-ins beyond HelloDino (MeshTools, CurveTools, AnalysisTools, under `plugins/`) with the SDK documented end to end in `docs/PLUGIN_SDK.md`. Remaining gap: the plug-in ABI (`include/dino8_plugin.h`) still has no Curve/Surface/Brep/Mesh *value* kind, so a plug-in flow node can generate document geometry via a command but cannot output geometry values other native nodes can wire further — see the Limitations section of `docs/PLUGIN_SDK.md`.
