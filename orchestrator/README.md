...
| `scheduler.py` | Trigger scripts on a cadence. Generates cron entries (Linux/macOS) **or** runs a portable Python daemon (any OS, incl. Windows), which writes a heartbeat the dashboard reads to show whether it's actually alive. |
| `dashboard.py` | A local web dashboard: per-module status with a **Run now** button and a log viewer, earnings (with a CSV export for bookkeeping), an interactive review queue (with a recently-resolved audit trail of past approve/reject decisions, also CSV-exportable), and an activity feed. |
